...
def filter_non_dataset_rows(input_csv='label_updated.csv', output_csv='label_test.csv'):
    ...
    print(f"Filtering rows where 'Kapak Görseli' does NOT start with 'dataset/'...")
    ...
    with open(input_csv, 'r', encoding='utf-8') as infile, \
        ...
        for row_num, row in enumerate(reader, start=2): 
            ...
            if not kapak_gorseli.startswith('dataset/'):
                writer.writerow(row)
                filtered_rows += 1
    ...
    print(f"Rows included (NOT starting with 'dataset/'): {filtered_rows}")
    print(f"Rows excluded (starting with 'dataset/'): {excluded_rows}")
    print()
...
def main():
    ...
    filter_non_dataset_rows(input_csv, output_csv)
